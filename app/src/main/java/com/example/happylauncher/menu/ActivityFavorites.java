    ...
    private void selectFavorites() {
        ...
        for (Application application : applications) {
            if (application instanceof Folder)
                app_names[i] = ((Folder) application).getDisplayNameWithCount();
        ...
        dialog.setPositiveButton(R.string.button_apply, (dialogInterface, position) -> {
            ...
            for (position = 0; position < selected.length; position++)
                if (selected[position])
                    file.writeLine(applications.get(position).getComponentInfo());
    ...
    public void onPause() {
        ...
        if (file.remove())
            for (Application application : favorites)
                file.writeLine(application.getComponentInfo());
        ...
        public void onBindViewHolder(@NonNull final FavoriteView favoriteView, int i) {
            ...
            favoriteView.itemView.setOnTouchListener((view, event) ->
            {
                ...
                if (event.getAction() == MotionEvent.ACTION_DOWN)
                    touchManager.startDrag(favoriteView);
                return false;
            });
