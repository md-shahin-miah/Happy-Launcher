...
import com.example.happylauncher.core.ApplicationsList;
import com.example.happylauncher.core.Folder;
import com.example.happylauncher.core.Search;
import com.example.happylauncher.events.PackagesListener;
import com.example.happylauncher.events.ShortcutLegacyListener;
import com.example.happylauncher.menu.DialogMenu;
import com.example.happylauncher.quickaccess.NotificationDisplayer;
...
public class ActivityMain extends AppCompatActivity implements View.OnClickListener, SharedPreferences.OnSharedPreferenceChangeListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		reverse_interface = sharedPreferences.getBoolean(Constants.REVERSE_INTERFACE, false) ;
		...
		if(sharedPreferences.getBoolean(Constants.NOTIFICATION, true)) notification.display(this) ;
			else notification.hide() ;

		// Define the width of an application item
		int app_size_pixels = Utils.getIconSize(this, sharedPreferences) ;
		int padding_pixels ;
		if(sharedPreferences.getBoolean(Constants.HIDE_APP_NAMES, false)
				&& sharedPreferences.getBoolean(Constants.REMOVE_PADDING, false)) padding_pixels = Math.round(app_size_pixels / 24f) ;
		...
		favoritesAdapter.setTextColor(Utils.getColor(sharedPreferences, Constants.TEXT_COLOR_FAVORITES, Constants.COLOR_FOR_TEXT_ON_OVERLAY)) ;
		...
		drawerAdapter.setTextColor(Utils.getColor(sharedPreferences, Constants.TEXT_COLOR_DRAWER, Constants.COLOR_FOR_TEXT_ON_OVERLAY)) ;
		...
		if(sharedPreferences.getBoolean(Constants.ALWAYS_SHOW_FAVORITES, false)) displayFavorites(true) ;
	...
	private void setApplicationTheme()
	{
		...
		String theme = sharedPreferences.getString(Constants.APPLICATION_THEME, Constants.NONE) ;
		switch(theme)
		{
			case "light" :
	...
	private void maybeForceOrientation()
	{
		...
		String forced_orientation = sharedPreferences.getString(Constants.FORCED_ORIENTATION, Constants.NONE) ;
	...
	private void toggleTouchTargets()
	{
		...
		if(sharedPreferences.getBoolean(Constants.TOUCH_TARGETS, false))
			{
				// Display or not touch targets according to the other settings
				if(sharedPreferences.getBoolean(Constants.ALWAYS_SHOW_FAVORITES, false))
						targetFavorites.setVisibility(View.GONE) ;
					else targetFavorites.setVisibility(View.VISIBLE) ;
				if(sharedPreferences.getBoolean(Constants.DISABLE_APP_DRAWER, false))
	...
	private void keepMenuAccessible()
	{
		...
		if(!(sharedPreferences.getBoolean(Constants.DISABLE_APP_DRAWER, false) ||
				sharedPreferences.getBoolean(Constants.ALWAYS_SHOW_FAVORITES, false))) return ;

		// Check if the menu button is visible
		if(!sharedPreferences.getBoolean(Constants.HIDE_MENU_BUTTON, false)) return ;

		// Browse all favorites
		String launcher = "{com.example.happylauncher/com.example.happylauncher.ActivityMain}" ;
		...
		int app_size = Utils.getIconSize(this, sharedPreferences) ;
		if(!sharedPreferences.getBoolean(Constants.REMOVE_PADDING, false)) app_size += Math.round(20 * density) ;
		if(!sharedPreferences.getBoolean(Constants.HIDE_APP_NAMES, false)) app_size += menu_button_height ;
		...
		if(display)
			{
				...
				int background_color = Utils.getColor(sharedPreferences, Constants.BACKGROUND_COLOR_FAVORITES, Constants.COLOR_FOR_OVERLAY) ;
				...
				if(sharedPreferences.getBoolean(Constants.HIDE_MENU_BUTTON, false)) menuButton.setVisibility(View.GONE) ;
			...
			else
			{
				...
				if(sharedPreferences.getBoolean(Constants.ALWAYS_SHOW_FAVORITES, false)) return ;
				...
				if(sharedPreferences.getBoolean(Constants.TRANSPARENT_STATUS_BAR, true))
						getWindow().setStatusBarColor(getResources().getColor(R.color.transparent)) ;
					else getWindow().setStatusBarColor(Utils.getColor(sharedPreferences, Constants.BACKGROUND_COLOR_FAVORITES, Constants.COLOR_FOR_OVERLAY)) ;
		...
		if(display)
			{
				...
				if(sharedPreferences.getBoolean(Constants.DISABLE_APP_DRAWER, false)) return ;

				// Update the recyclers (favorites panel and applications drawer) if needed
				if(adapters_update_needed) updateAdapters() ;

				// Color the system bars and the drawer background
				int background_color = Utils.getColor(sharedPreferences, Constants.BACKGROUND_COLOR_DRAWER, Constants.COLOR_FOR_OVERLAY) ;
			...
			else
			{
				...
				int favorites_background_color = Utils.getColor(sharedPreferences, Constants.BACKGROUND_COLOR_FAVORITES, Constants.COLOR_FOR_OVERLAY) ;
				...
				if(reverse_interface && sharedPreferences.getBoolean(Constants.ALWAYS_SHOW_FAVORITES, false))
	...
	private void maybeHideSystemBars(boolean force_display)
	{
		...
		if(sharedPreferences.getBoolean(Constants.DARK_STATUS_BAR_ICONS, false))
			{
		...
		if(!force_display && sharedPreferences.getBoolean(Constants.IMMERSIVE_MODE, false))
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key)
	{
		...
		switch(key)
		{
			...
			case Constants.TEXT_COLOR_FAVORITES :
				...
				favoritesAdapter.setTextColor(Utils.getColor(this.sharedPreferences, Constants.TEXT_COLOR_FAVORITES, Constants.COLOR_FOR_TEXT_ON_OVERLAY)) ;
				break ;
			case Constants.TEXT_COLOR_DRAWER :
				// Update the text color of the drawer
				drawerAdapter.setTextColor(Utils.getColor(this.sharedPreferences, Constants.TEXT_COLOR_DRAWER, Constants.COLOR_FOR_TEXT_ON_OVERLAY)) ;
				break ;
			...
			case Constants.NOTIFICATION :
				...
				if(this.sharedPreferences.getBoolean(Constants.NOTIFICATION, true))
					{
			...
			case Constants.REVERSE_INTERFACE :
				...
				reverse_interface = this.sharedPreferences.getBoolean(Constants.REVERSE_INTERFACE, false) ;
				if(reverse_interface) setContentView(R.layout.activity_main_reverse) ;
					else setContentView(R.layout.activity_main) ;
				recreate() ;
				updateList(this) ;
	...
	class GestureListener extends GestureDetector.SimpleOnGestureListener
	{
			...
			if((Math.abs(y_distance) > Math.abs(x_distance)) && (Math.abs(y_distance) > swipe_trigger_distance))
				{
					...
					if(sharedPreferences.getBoolean(Constants.ALWAYS_SHOW_FAVORITES, false) && swipe_drawer)
						{
							displayDrawer(true) ;
							return true ;
						}
			...
			if((Math.abs(x_distance) > Math.abs(y_distance)) && (Math.abs(x_distance) > swipe_trigger_distance))
				{
					...
					return Utils.searchAndStartApplication(homeScreen, sharedPreferences, swipe_direction_setting_key) ;
				}
		...
		{
			return Utils.searchAndStartApplication(homeScreen, sharedPreferences, Constants.DOUBLE_TAP) ;
		}
